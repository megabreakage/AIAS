...
use App\Http\Requests\Tenant\SectionStyles\CreateSectionStyleRequest;
use App\Http\Requests\Tenant\SectionStyles\UpdateSectionStyleRequest;
...
final class SectionStyleController extends BaseApiController
{
    ...
    public function store(CreateSectionStyleRequest $request): JsonResponse
    {
        ...
        Log::info('Creating section style', ['name' => $data['name'], 'tenant_id' => $data['tenant_id']]);

        $sectionStyle = DB::transaction(function () use ($data): SectionStyle {
        ...
        return $this->success(
            SectionStyleResource::make($sectionStyle->load(['creator', 'updater']))->resolve(),
    ...
    public function update(UpdateSectionStyleRequest $request, string $identifier): JsonResponse
    {
        ...
        $sectionStyle = DB::transaction(function () use ($identifier, $data): SectionStyle {
    ...
    public function restore(string $identifier): JsonResponse
    {
        ...
        $sectionStyle = DB::transaction(function () use ($identifier): SectionStyle {
